package binary.wz.gateway.filter;

import binary.wz.gateway.config.IgnoreUrlsConfig;
import binary.wz.gateway.handler.AuthExceptionHandler;
...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (StringUtils.isBlank(access_token)) {
            return authExceptionHandler.writeError(exchange, "请登录");
        }
        // 校验 token 是否有效
        String checkTokenUrl = "http://hawk-oauth-server/oauth/check_token?token=".concat(access_token);
        try {
            // 发送远程请求，验证 token
            ResponseEntity<String> entity = restTemplate.getForEntity(checkTokenUrl, String.class);
            // token 无效的业务逻辑处理
            if (entity.getStatusCode() != HttpStatus.OK) {
                return authExceptionHandler.writeError(exchange,
